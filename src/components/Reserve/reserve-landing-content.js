export const overnight = {
	image: '/assets/overnight.jpg',
	title: 'Overnight Reservations',
	content:
		'Come and enjoy an overnight stay at Ananda Ashram.  Registrations accepted until 4 pm the same day. Choose this option to book stays of one to six nights.  Our 24-hour guest rates are per person and include room and board and the Ashram Core Program, consisting of daily...'
};

export const fourweek = {
	image: '/assets/fourweek.jpg',
	title: 'Extended Lodgings - 4 Weeks',
	content:
		'"Happiness is the automatic and natural state of consciousness. One does not have to bring it from the outside." (Shri Brahmananda Sarasvati). Realign yourself to your spiritual&nbsp;path with a month-long retreat at Ananda Ashram, Choose this option to book stays of...'
};
