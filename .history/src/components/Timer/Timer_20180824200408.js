...
export default class Timer extends Component {
	constructor(props) {
		...
		this.state = {
			minutes: this.props.navigation.getParam('min'),
			interval: this.props.navigation.getParam('int'),
			isTimer: false,
			isInterval: false,
			tick: 0,
			ambChecked: this.props.navigation.getParam('music'),
			isModal: false,
			txtInput: '',
			error: false
		};
	...
	static navigationOptions = ({ navigation }) => {
		navigation.addListener('willFocus', () => {
			console.log('static: ' + JSON.stringify(navigation.state.params));
		});
	};
	...
	render() {
		if (!this.props.navigation.state.params) {
			...
			console.log('state mins: ' + this.state.minutes);
		}
		return (
			<View style={{ flex: 1 }}>
					...
					<View style={styles.sliderWrap}>
						{this.props.navigation.state.params ? (
							<Text style={styles.sliderLabel} key={1}>
								Meditation Time: {this.props.navigation.state.params.min} mins
							</Text>
						) : (
						...
						{this.props.navigation.state.params ? (
							<Slider
								value={this.props.navigation.state.params.min}
								minimumValue={0}
								maximumValue={180}
								step={5}
						...
						{this.props.navigation.state.params ? (
							<Text style={styles.sliderLabel}>Interval: {this.props.navigation.state.params.int} mins</Text>
						) : (
							<Text style={styles.sliderLabel}>Interval: {this.state.interval} mins</Text>
						)}
						{this.props.navigation.state.params ? (
							<Slider
								value={this.props.navigation.state.params.int}
								minimumValue={0}
								maximumValue={60}
								step={5}
								minimumTrackTintColor="#ffcd32"
