...
export default class Timer extends Component {
	...
	handleTimer = () => {
		const m = this.props.navigation.state.params.min;
		const ivl = this.props.navigation.state.params.int;
		const mus = this.props.navigation.state.params.music;
		...
		if (m > 0) {
			...
			time = m * 60000;
			console.log('time ' + time + ' min ' + m + ' int ' + ivl + ' music ' + mus);
			console.log('state tick ' + this.state.tick);
		} else {
		...
		if (ivl > 0) {
			console.log('int > 0 ' + ivl);
			this.setState({ interval: ivl }, () => {});
		}
