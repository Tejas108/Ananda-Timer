...
let ticker;
let count = 0;
// let test = JSON.stringify(this.props.navigation.state.params);
export default class Timer extends Component {
	constructor() {
		...
		this.state = {
			...
			txtInput: '',
			error: false
			// preset: JSON.stringify(this.props.navigation.state.params)
		};
	}

	static navigationOptions = ({ navigation }) => {
		navigation.addListener('didFocus', () => {
			// var minutes = this.props.navigation.state.params.min;
			// console.log(minutes);
		});
	};
	...
	handlePreset = () => {
		let preset = JSON.stringify(this.props.navigation.state.params);
	...
	render() {
		var minutes = this.props.navigation.state.params;
		console.log(minutes);
		this.setState({ preset: minutes });
		return (
			<View style={{ flex: 1 }}>
