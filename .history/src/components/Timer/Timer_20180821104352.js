...
export default class Timer extends Component {
	...
	handleSaveSettings = () => {
		console.log('save setting calledd');
		realm.write(() => {
			let ID = realm.objects('Presets').length + 1;

			realm.create('Presets', {
				preset_id: ID,
		...
		this.setState({ txtInput: '', isModal: !this.state.isModal });
	};
	...
	render() {
		var A = realm.objects('Presets');
		var myJSON = JSON.stringify(A);
		console.log('objects: ' + myJSON);
		return (
			<View style={{ flex: 1 }}>
