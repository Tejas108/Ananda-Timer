...
export default class Timer extends Component {
	...
	handleTimer2 = (m, int, mu) => {
		console.log('timer 2');
		console.log(m, int, mu);
		let currTimer = this.state.isTimer;
		this.setState({ isTimer: !currTimer, tick: m }, () => {});
		console.log(int + ' ' + mu);
		let time = m * 60000;
		mTimeout = setTimeout(this.handleTimerEnd, time);
		ticker = setInterval(this.handleTicker2(this, m), 60000);
		if (mu) {
			this.reloadPlayer();
			this.ambPlayer.play();
			this.ambPlayer.looping = true;
		...
		if (int > 0) {
			console.log('int > 0');
			this.handleInterval2(this, int);
		}
		this.forceUpdate();
	};

	handleTicker2 = m => {
		console.log('handleTicker2 called');
		if (m > 0) {
			count = count + 1;
			this.setState({ tick: m - count });
		}
	};

	handleInterval2 = int => {
		console.log('handleInterval2 called');
		this.setState({ isInterval: true });
		let interval = int * 60000;
		let iInterval = setInterval(this.handleIntBell, interval);
	};

	handleTicker = () => {
		console.log('handleTicker called');
					...
					<View style={styles.sliderWrap}>
						...
						{params ? (
							<Button
								...
								fontFamily={'Helvetica'}
								fontSize={24}
								onPress={this.handleTimer2.bind(this, params.min, params.int, params.music)}
