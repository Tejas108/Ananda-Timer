...
export default class Timer extends Component {
	...
	handleSaveSettings = async () => {
		//this.handleModal();
		//console.log('save setting called');
		...
		try {
			const existingPresets = await AsyncStorage.getItem(presets);
			let newPreset = JSON.parse(existingPresets);
			if (!newPresets) {
				newPreset = [];
			}
		...
		await AsyncStorage.setItem('presets', JSON.stringify(newPreset))
			.then(() => {
		...
		// store.push('presets', preset).catch(error => {
		// 	console.error(error.message);
		// });
		//console.log('store ' + store.presets[0]);
